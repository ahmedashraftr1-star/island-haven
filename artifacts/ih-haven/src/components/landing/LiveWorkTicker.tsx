 ...
 * member work (name · tech · title) from /api/works, now scrolling OVER a dimmed
 * photograph of the space so the live output reads as "the community at work,"
 * not a chat log. Every entry is real and links to the member's profile. Reuses
 * the `ih-marquee` keyframe; second row reversed. Pauses on hover; reduced-motion
 * → static. Renders nothing if there isn't enough real work to fill a marquee.
 */
...
function TickerItem({ e, accent }: { e: Entry; accent: "red" | "sand" }) {
  return (
    <Link href={`/u/${e.authorId}`} className="group/item inline-flex items-center whitespace-nowrap">
      <span className="font-display font-bold text-white text-[1.02rem] tracking-tight transition-colors group-hover/item:text-primary">
        {e.member}
      </span>
      <span className="mx-3 text-white/25 text-sm" aria-hidden>·</span>
      <span className="font-mono text-white/50 text-[0.78rem] tracking-wide">{e.tech}</span>
      <span className="mx-3 text-white/25 text-sm" aria-hidden>·</span>
      <span className="text-white/75 text-[0.95rem]">{e.title}</span>
      <span className={`mx-7 text-lg ${accent === "red" ? "text-primary/55" : "text-sand/55"}`} aria-hidden>✦</span>
    </Link>
  );
}
...
export function LiveWorkTicker() {
  ...
  return (
    <CinematicMedia
      as="section"
      src={imageUrl("/photos/IMG_8349.webp")}
      scrim="heavy"
      sideScrim={false}
      className="group border-y border-white/[0.08]"
      aria-label={t({ ar: "أعمال حيّة من المجتمع", en: "Live work from the community" })}
    >
      <div className="flex items-center justify-between px-[clamp(1rem,4vw,2.5rem)] py-3.5 border-b border-white/[0.1]">
        <span className="font-mono text-[0.68rem] tracking-[0.14em] uppercase text-white/55">Live work output</span>
        <span className="flex items-center gap-2 font-mono text-[0.68rem] tracking-[0.06em] uppercase text-white/70">
          <span
